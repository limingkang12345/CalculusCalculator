...
- **功能概览**：
  ...
  - **表达式缓存**（v1.6.4）：通过缓存区管理常用表达式，支持对话框选择和标签页浏览
  - **可视化公式输入**（v1.6.4）：基于 pyqt5-math-widget，在输入框中以图形化方式插入数学公式
  - **存档设置**（v1.6.4）：设置页新增存档选项，可精细控制存档时保存哪些类型的数据
...
2. **可视化公式输入**：基于 pyqt5-math-widget（已内置于 `math_input/` 目录），支持结构化选择和编辑数学符号
3. **存档设置**：设置页新增"存档设置（保存时）"列表，可勾选/取消函数列表、方程列表、缓存区等内容，精细控制存档范围
...
16. **表达式缓存与可视化输入**（v1.6.4 新增）  
    缓存区可暂存常用表达式，支持弹窗选择与标签页管理；基于 pyqt5-math-widget 的图形化公式输入面板，无需记忆 LaTeX 即可插入数学符号。
...
│   ├── zh_CN.ts         # 中文翻译源
│   └── en_US.ts         # 英文翻译源
├── i18n/                # 编译后的翻译文件（.qm），打包时复制到 exe 同级
└── docs/                # 文档源码（MkDocs 构建）
    ├── index.md         # 首页
    ├── about.md         # 关于 / 版本历史
